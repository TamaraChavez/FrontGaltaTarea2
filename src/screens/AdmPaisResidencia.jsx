...
const AdmPaisResidencia = () => {
  ...
  useEffect(() => {
   ...
   const handleDelete = async (idPais) => {
    ...
    try {
      ...
      console.log('Pais eliminado:', data.message);
    
      // Actualiza el estado de clientes en la UI después de la eliminación exitosa
      setPaises(prevPaises=> prevPaises.filter(pais => pais.idPais !== idPais));
    } catch (error) {
  ...
  return (
    ...
    <ContenedorTabla>
      <h1>Administación de paises de residencia</h1>
      <BotonAgregar as={Link} to="/AdmPaisResidencia/FormPaisResidencia">Nuevo</BotonAgregar>
      <Table>
        <thead>
          <Tr>
            <Th>ID</Th>
...
export default AdmPaisResidencia;
