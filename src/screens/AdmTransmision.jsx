...
const AdmTransmision = () => {
  ...
  const handleDelete = async (idTransmision) => {
    ...
    try {
      ...
      setTransmision(prevTransmisiones=> prevTransmisiones.filter(transmision => transmision.idTransmision !== idTransmision));
    } catch (error) {
      console.error('Error al eliminar la transmision:', error);
      alert(`Error al eliminar la transmision: ${error.message}`);
    }
  };
  ...
  return (
    ...
    <ContenedorTabla>
      <h1>Administación de transmisiones</h1>
      <BotonAgregar as={Link} to="/AdmTransmision/FormTransmision">Nuevo</BotonAgregar>
      <Table>
        <thead>
          <Tr>
            <Th>ID</Th>
