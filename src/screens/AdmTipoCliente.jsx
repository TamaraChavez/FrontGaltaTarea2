...
const AdmTipoCliente = () => {
  ...
  const handleDelete = async (idTipoCliente) => {
    ...
    try {
      ...
      console.log('Tipo Cliente eliminado:', data.message);
    
      // Actualiza el estado de clientes en la UI después de la eliminación exitosa
      setTipoClientes(prevTipoCliente=> prevTipoCliente.filter(tipoCliente => tipoCliente.idTipoCliente !== idTipoCliente));
    } catch (error) {
      console.error('Error al eliminar el tipo de cliente:', error);
      alert(`Error al eliminar el tipo cliente: ${error.message}`);
    }
  };
  ...
  return (
    ...
    <ContenedorTabla>
      <h1>Administación de tipo de clientes</h1>
      <BotonAgregar as={Link} to="/AdmTipoCliente/FormTipoCliente">Nuevo</BotonAgregar>
